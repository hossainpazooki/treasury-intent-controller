...
// --- CONTRACT.md §2.4: constructor + full client fail-closed matrix ---
...
func TestNewHTTPScorerDefaults(t *testing.T) {
	...
	if DefaultTimeout != 5*time.Second {
		t.Fatalf("DefaultTimeout = %v, want 5s (CONTRACT.md §2.4)", DefaultTimeout)
	}
...
func TestHTTPScorerFailClosedMatrix(t *testing.T) {
	// Every remaining row of the §2.4 client matrix maps to Unevaluable. The
	// refused/500/garbage/unknown-result rows are covered by the slice-1 tests
	// above (kept verbatim per CONTRACT.md §5.3).
...
func TestHTTPScorerRequestCarriesAllFields(t *testing.T) {
	// Happy path: the marshaled request carries all seven §2.4 fields, and the
...
// --- CONTRACT.md §9: cross-language golden fixtures ---
