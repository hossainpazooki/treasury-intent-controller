...
| [2026-08-14-completed-ban-is-prose-not-gate.md](2026-08-14-completed-ban-is-prose-not-gate.md) | verified | the COMPLETED ban is CONTRACT §1.3 prose + a zero-grep, NOT a contractcheck gate (vocab_test pins only actor nouns + the retired proper noun) -- prose authors get no tripwire; a skeptic overturned a finding claiming otherwise |
| [2026-08-14-intent-declare-exit0-includes-shadow.md](2026-08-14-intent-declare-exit0-includes-shadow.md) | verified | intent-declare exit 0 includes SHADOW_RECORDED (recorded, NOT authorized) and the CLI package has no test files -- scripts must parse class=, demos must never equate exit 0 with authorization |
| [2026-08-14-sdk-public-monorepo-private.md](2026-08-14-sdk-public-monorepo-private.md) | verified | intent-plane is PUBLIC, this monorepo private (gh-verified) -- every SDK commit is instantly public marketing surface; handoffs/ledgers/drafts stay here; pkg.go.dev prerequisite already satisfied |
| [2026-08-16-atlas-codename-residual.md](2026-08-16-atlas-codename-residual.md) | verified | public SDK repo still carries ATLAS on 26 lines/6 files incl. SCORER_ATLAS_* env vars -- full scrub is a breaking config rename, operator ruling needed, not a doc edit |
| [2026-08-16-plain-gobuild-irreproducible.md](2026-08-16-plain-gobuild-irreproducible.md) | verified | same machine/commit/toolchain: plain vs -trimpath go build hashes differ (checkout path embedded) -- reproduce-my-hash docs must pin flags + toolchain version |
| [2026-08-16-buildid-defeats-trimpath-reproducibility.md](2026-08-16-buildid-defeats-trimpath-reproducibility.md) | verified | -trimpath alone is NOT enough: -buildvcs=false + -ldflags=-buildid= are each load-bearing (59-byte build-ID delta, content ID already identical); corrects the prior entry's flag set and refutes its recorded hash |
| [2026-08-16-atlas-scrub-executed.md](2026-08-16-atlas-scrub-executed.md) | verified | operator overturned the deferral: ATLAS/COMPASS/monorepo refs now ZERO tracked, env vars renamed shim-free (silent-not-verify risk accepted + documented); exposed a LARGER internal-ref class (own ADR numbers ~15 refs; a third private project ~8) left untouched |
| [2026-08-17-bang-in-commit-subject-breaks-git-bash.md](2026-08-17-bang-in-commit-subject-breaks-git-bash.md) | verified | a conventional-commits `!` marker in an emitted `git commit -m "..."` dies to bash history expansion (inside double quotes), spilling the body as commands and leaving files staged-but-uncommitted -- emit `set +H`, a heredoc, or no marker |
| [2026-08-18-live-verifier-refutes-adapter-seed-reuse.md](2026-08-18-live-verifier-refutes-adapter-seed-reuse.md) | verified | the LangChain adapter's key-derived episode seed redeclared one intent id on same-args retries; 30 lane tests + 7 pins + 2 skeptic passes all green over it -- only the LIVE verifier caught the intent-seq-dup, because per-intent contiguity exists only at the system level; fixed with a per-invocation nonce, regression pinned, 12/12 both lanes |
| [2026-08-18-git-checkout-revert-clobbers-earlier-edit.md](2026-08-18-git-checkout-revert-clobbers-earlier-edit.md) | verified | reverting a planted mutation with `git checkout -- <file>` restores the INDEX state, not the pre-mutation state -- it silently wiped an earlier uncommitted real edit in the same file; revert plants by editing them back out, then re-run the cheapest whole-tree check |
| [2026-08-18-py314-function-local-models-break-tool-schemas.md](2026-08-18-py314-function-local-models-break-tool-schemas.md) | verified | py3.14 deferred annotations (annotationlib): a pydantic model defined inside a test function raises NameError when langchain-core builds a tool's args schema -- per-test models for tool signatures must live at module level |
